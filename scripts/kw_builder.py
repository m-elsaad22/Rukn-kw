...
# City-level search intent so six pages of the same service do not rank for one query.
CITY_INTENT = {
    "kuwait": (
        "تنفيذ داخل العاصمة والأبراج",
        "من يبحث هنا يريد {noun} يناسب الأبراج والمكاتب والبيوت القديمة في الوسط، مع تنسيق المصعد والوقوف في الشرق والصالحية.",
    ),
    "hawalli": (
        "حل عملي للشقق والكثافة الساحلية",
        "النية الشائعة هي {noun} داخل شقق السالمية وحولي: زحام، مضخات عمارات، ورذاذ بحري يغيّر الخامة أسرع من الضاحية.",
    ),
    "farwaniya": (
        "بيوت ومحلات بحركة يومية",
        "البحث في الفروانية يميل إلى {noun} بعد دوام المحل أو وسط عمائر مزدحمة، مع شبكات أقدم وغبار أعلى من الساحل.",
    ),
    "mubarak-al-kabeer": (
        "فلل وأحواش وملاحق",
        "في مبارك الكبير النية غالباً {noun} لفيلا أو حوش أو ملحق، لا لممر عمارة، مع سطح وخزان أقرب للفحص.",
    ),
    "al-ahmadi": (
        "مساحات وأسطح ورطوبة ساحلية",
        "الباحث في الأحمدي يريد {noun} لمساحات أوسع: أسطح كبيرة، مسافات بين الأحياء، ورطوبة الفنطاس والفحيحيل مقابل غبار الداخل.",
    ),
    "al-jahra": (
        "غبار صحراوي ومساحات واسعة",
        "النية في الجهراء هي {noun} مع غبار يلتصق بالخزان والدكت، وأحواش ترابية، ومسافة تجعل تأكيد صباح التنفيذ أوضح من موعد فضفاض.",
    ),
}
...
def build(post: dict, catalog: dict) -> dict:
    ...
    intent_label, intent_body = CITY_INTENT.get(city_key, CITY_INTENT["kuwait"])
    intent_body = intent_body.format(noun=noun)
    intros = [
        f"نيّة البحث في هذه الصفحة: {intent_label}. {intent_body} {spec['problem']}",
        f"{intent_body} {city['buildings']}",
        f"{intent_body} {city['climate']}",
        f"{spec['audience']} يصلون إلى هذه الصفحة بنيّة {intent_label}. {spec['problem']}",
        f"{intent_body} السؤال العملي: هل تحتاج {noun} الآن أم معاينة مكتوبة أولاً؟",
        f"{intent_body} {city['access']}",
    ]
...
def _features(noun, city, spec, seed):
    ...
    titles = [
        ...
        ("تواصل مباشر", "عبر صفحة التواصل دون وسيط إعلانات."),
...
def _faqs(kw, noun, city, spec, areas, tech, seed):
    ...
    base = [
        ...
        {"question": "كيف أحجز؟", "answer": f"من صفحة التواصل على الموقع. اذكر {noun} ومنطقتك مثل {areas[0]} وصورة للنقطة إن أمكن."},
...
def _long_unique_blocks(noun, city, spec, outline, areas_txt, company_line, seed):
    ...
    extra_p = (
        f"<p>في {escape(a0)} نربط {escape(spec['signs'][2])} بما يظهر على الطبيعة، "
        f"ولا نعمم نتيجة بيت في {escape(a1)} على وحدة مختلفة التشطيب داخل {escape(city['label'])}.</p>"
    )
...
def _compose_sections(**k):
    ...
    sections.append(
        '<div class="rukn-hero">'
        f'<p class="hero-label">{escape(city["label"])} · {escape(k["intent_label"])}</p>'
        f"<p>{escape(k['what_p'][:280])}</p>"
        '<p class="rukn-hero-actions">'
        f'<a class="rukn-btn rukn-btn-wa" href="{CONTACT}"><i class="fa-brands fa-whatsapp"></i> {escape(CTA)}</a>'
        f'<a class="rukn-btn" href="{CONTACT}"><i class="fa-brands fa-whatsapp"></i> واتساب</a>'
        "</p></div>"
    )
    ...
    block_close = [
        ...
        f'<p class="rukn-hero-actions"><a class="rukn-btn rukn-btn-wa" href="{CONTACT}"><i class="fa-brands fa-whatsapp"></i> {escape(CTA)}</a>'
        f'<a class="rukn-btn" href="{CONTACT}"><i class="fa-brands fa-whatsapp"></i> واتساب</a></p>'
        "</div>",
    ]
    extra_all = _long_unique_blocks(
        k["noun"], k["city"], k["spec"], k["outline"], k["areas_txt"], k["company_line"], k["seed"]
    )
    heading_extras = extra_all[:8]
    extra_tail = extra_all[8:]
    extra = [heading_extras[(k["outline"] + i * 2) % 8] for i in range(3)] + extra_tail
    family_all = family_sections(
        k["fam"], k["noun"], k["city"], k["spec"], k["areas"], k["outline"], k["seed"]
    )
    family = [family_all[(k["outline"] + i) % len(family_all)] for i in range(min(2, len(family_all)))]
